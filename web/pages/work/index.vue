<template>
  <article class="p-2 pt-12 flex flex-col space-y-2">
    <NuxtLink v-for="project in page.projects" :key="project._id" :to="'/work/' + project.slug.current" class="flex">
      <Media :media="project.media" class-names="w-[30rem]" />
      <h2 class="pl-2">
        {{ project.title }}
      </h2>
    </NuxtLink>
...
useSeo({ general: data.value.general, title: page.value.projectsOverview.title, seo: page.value.projectsOverview.seo });
</script>
